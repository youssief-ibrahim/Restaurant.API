...
namespace Restaurant.Application.Services.DeliverySR
{
    public class DeliveryServices : IDeliveryServices
    {
        ...
        public DeliveryServices(IGenericRebosatory<Delivery> GenDelivary, IGenericRebosatory<Order> GenOrder, IUnitOfWork unitOfWork, IMapper mapper)
        {
        ...
        {
            var order = await GenOrder.GetById(o => o.OrderId == orderid);
            if (order == null)
                throw new NotFoundException($"Not Found Order with this Id {orderid}");
            var existingDelivery = await GenDelivary.GetById(d => d.OrderId == orderid);
            ...
            try
            {
                await GenDelivary.Create(delivery);
        ...
        {
            var delivery = await GenDelivary.GetById(d => d.OrderId == orderid && d.UserId == userid);
            if (delivery == null)
        ...
        {
            var res=await GenDelivary.GetAll();
            if(!res.Any())
        ...
        {
            var res= await GenDelivary.GetById(d => d.DeliveryId == id);
            if (res == null)
        ...
        {
           var res =await GenDelivary.GetById(d => d.OrderId == orderid);
            if (res == null)
