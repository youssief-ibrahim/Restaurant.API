...
namespace Restaurant.Application.Services.PaymentSR
{
    public class PaymentService : IPaymentService
    {
        ...
        public PaymentService(IConfiguration config,IUnitOfWork unitOfWork,IMapper mapper)
        {
            this.config = config;
        ...
        public async Task<PaymentDTO> CreateOrUpdatePaymentIntentAsync(int cartId)
        {
            ...
            {
                var meal = await unitOfWork.Genunit<Meal>().GetById(m => m.MealId == item.MealId);
            ...
            if (string.IsNullOrEmpty(cart.PaymentIntentId))
            {
                ...
                var payment = await unitOfWork.Genunit<Payment>().GetById(p =>
                ...
                unitOfWork.Genunit<Payment>().update(payment);
            }
            else
            {
        ...
        {
            var payment = await unitOfWork.Genunit<Payment>().GetById(
            ...
            var order = await unitOfWork.Genunit<Order>().GetById(o => o.OrderId == payment.OrderId);
            order.Status = OrderStatus.Confirmed;
            unitOfWork.Genunit<Order>().update(order);

          
            var orderItems = await unitOfWork.Genunit<OrderItem>().FindAll(i => i.OrderId == order.OrderId, i => i.Meal);
            foreach (var item in orderItems)
            {
                item.Meal.Quantity -= item.Quantity;
                unitOfWork.Genunit<Meal>().update(item.Meal);
            }

          
            //var cart = await Gencart.GetById(c => c.CustomerId == order.CustomerId); is also okkay
            var cart = await unitOfWork.Genunit<Cart>().GetById(c => c.OrderId == order.OrderId);
            var cartItems = await unitOfWork.Genunit<CartItem>().FindAll(ci => ci.CartId == cart.CartId);
